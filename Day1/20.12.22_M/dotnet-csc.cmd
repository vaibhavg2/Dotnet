...
for %%f in ("%dotnetlib%\System.*.dll") do (
	set x=%%f
	if not "!x:~-11!"==".Native.dll" echo -r:"%%f" >> %temp%\csc-%fwkver%.rsp
)
:compile
set prog=%~n1
dotnet "%dotnethome%sdk\%sdkver%\Roslyn\bincore\csc.dll" -out:%prog%.dll -d:NETCOREAPP -nologo @%temp%\csc-%fwkver%.rsp %*
if %errorlevel% == 0 (
    if exist %prog%.dll (
	for %%a in (%*) do (
		set x=%%a
